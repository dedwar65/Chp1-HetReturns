...
%\vspace{5mm}
  ...
  \par Estimate a common rate of return across households by finding the $\grave{R}$ which matches the capital-to-output ($\frac{K}{Y}$) ratio from data.
  \vspace{2.5mm}

  \item Ex-ante heterogeneity: $R$\textit{-dist} model
  \par Estimate  a \textbf{Uniform distribution} of returns across households by finding the $\grave{R}$,$\nabla$ which matches empirical Lorenz targets, given $\frac{K}{Y}$.
  \end{enumerate}
...
\small
\par Empirical targets using 2004 SCF data \vspace{2mm}
...
\vspace{-7.5mm}
 \begin{eqnarray*}
  ...
  &\text{s.t.}& \\
  \frac{K}{Y} = 7.47. %\frac{K_{PF}}{Y_{PF}}.
\end{eqnarray*}
